        ...
        const query = `
            SELECT 
                ...
                CASE 
                    ...
                    WHEN a.user_type = 'employer' THEN CONCAT('BCC-UMP-EMP-', LPAD(emp.id::text, 9, '0'))
                END as user_id,
                CASE 
...
// =====================================================================
// --- EMPLOYERS MANAGEMENT (FORMATTED 9-DIGIT ID) ---
// =====================================================================
router.get('/employers', async (req, res) => {
    try {
        ...
        let pocData = [];
        try {
            // Attempt to fetch extra PoCs if the table exists
            const pocsResult = await pool.query("SELECT * FROM employer_hrs");
            pocData = pocsResult.rows;
        } catch (err) {
            console.log("No employer_hrs table found or empty, skipping extra PoCs.");
        }
        ...
        try {
            ...
            const jobsResult = await pool.query(`
                ...
                FROM jobs 
                WHERE LOWER(status) = 'approved' OR LOWER(status) = 'active' 
            ...
            console.log("No jobs found or error counting jobs.");
        }

        // Map database columns exactly to what the frontend expects
        const formattedData = result.rows.map(e => {
            const empId = e.id;
            const empName = e.company_name || e.name || 'Unnamed Company';
            
            // Fallbacks for email & phone
            const empEmail = e.email || e.poc1_email || e.contact_email || 'No Email';
            const empPhone = e.phone || e.poc1_phone || e.mobile || 'No Phone';
            
            // GST verification
            const rawGst = e.gst_number || e.gst || 'Unverified';
            const empGst = (e.is_gst_verified === true || rawGst !== 'Unverified') ? 'Verified' : 'Pending';
            
            // Status normalization
            let empStatus = (e.status || 'pending').toLowerCase();
            ...
            // FORMATTED EXACTLY LIKE CANDIDATE IDs
            return {
                id: `BCC-UMP-EMP-${String(empId).padStart(9, '0')}`, 
                dbId: empId, 
                name: empName,
                email: empEmail,
                phone: empPhone,
...
// =====================================================================
// --- STALL-WISE INTERVIEW CONTROL & LOGS REPORT ---
// =====================================================================
...
router.get('/exhibitors', async (req, res) => {
    try {
        ...
        const formattedData = result.rows.map(e => ({
            id: `BCC-UMP-EXH-${String(e.id).padStart(9, '0')}`,
            dbId: e.id,
            name: e.name,
            email: e.email || 'N/A',
            phone: e.phone || 'N/A',
